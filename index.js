...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:MINATO\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=11934488829:+55 11 9344 88829\n' // GANTI NOMOR LU
            + 'END:VCARD'
...
baby.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ *BEM VINDO AO GRUPO ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Koe Rlx Nesse Krlho É Olha A Regra Do Grupo Para N Ser Removido \n *___________________________*\
...
*/
				...
				nul = body.slice(7)
				reply('「❗」Espera ae mn carregando aqui
				...
				ramlan = body.slice(8)
				reply('「❗」Espere ae mn carregando aqui')
		...
		case 'textlight':
		    ...
		    break
				...
				if (args.length < 1) return baby.sendMessage(from, 'Onde está o código do idioma Sis? exemplo : ${prefix}tts id Olá Screamo', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('O texto é muito longo Mn')
                ...
                var gh = body.slice(12)
					...
					baby.sendMessage(from, buffer, image, {caption: 'Aqui Gay >_<', quoted: mek})
					await limitAdd(sender)
					break
                case 'meme':
                if (isBanned) return reply(nad.baned())
...
// PREMIUM
		 ...
		 case 'tomp3':
				...
				if (!isPrem) return reply(nad.premium())
                	baby.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('*Responda ao vídeo-_-*')
					reply(nad.wait())
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Falhou Humm:(')
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*O Recurso Bem Vindo Está Ativo*')
						welkom.push(from)
           ...
           case 'setname':
                ...
                baby.sendMessage(from, '*「 Sucesso 」Descricão Alterada Com Sucesso', text, {quoted: mek})
					break
           case 'demote':
                if (!isRegistered) return reply(nad.noregis())           
					if (!isGroup) return reply(nad.groupo())
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Não Mamo Direito Perdeu Admin Parca*🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
                ...
                if (!isRegistered) return reply(nad.noregis())				
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Esse Mamo O Admin Acabou De Ser Promovido Pra Adm>_< :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
...
*/
				...
				case 'bcgc':
				     if (!isOwner) return reply(nad.ownerb())
					if (args.length < 1) return reply('Cadê O Texto Meu Chefe>_<')
